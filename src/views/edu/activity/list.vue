<template>
  <div class="app-container">
    ...
    <el-form :inline="true"
      ...
      <el-form-item>
        <el-input v-model="activityQuery.title"
                  placeholder="活动标题" />
      </el-form-item>
      <el-form-item label="添加时间">
        <el-date-picker v-model="activityQuery.begin"
                        type="datetime"
                        placeholder="选择开始时间"
      ...
      <el-form-item>
        <el-date-picker v-model="activityQuery.end"
                        type="datetime"
                        placeholder="选择截止时间"
                        value-format="yyyy-MM-dd HH:mm:ss"
                        default-time="00:00:00" />
      ...
      <el-table-column label="操作"
        ...
        <template slot-scope="scope">
          ...
          <router-link :to="'/activity/info/'+scope.row.id">
            <el-button type="primary"
                       size="mini"
                       icon="el-icon-edit">修改活动信息</el-button>
          </router-link>
...
export default {
  ...
  methods: { // axios方法
    ...
    getList (current = 1) {
      ...
      activity.getActivityList(this.current, this.limit, this.activityQuery)
        .then(response => {
    ...
    resetData () { // 清空条件查询
      // 表单输入项清空
      this.activityQuery = {} // 因为是双向绑定，所以此处对象为空，表单中也会为空
      // 查询所有讲师数据
      this.getList()
    },
